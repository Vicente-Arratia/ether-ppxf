...
with open(f'/home/ether/ether-ppxf/out/fitting_pipeline/{sample}/results.csv', 'a') as results:
    for index, source in enumerate(sources_list[:]):
        ...
        # Get sigma value from SDSS database
        sdss_table = Table(SDSS.query_region(SkyCoord(ra = header['RA'], dec = header['DEC'], unit = 'deg'), radius = 3*u.arcsec, spectro = True, specobj_fields = ['velDisp', 'velDispErr'], data_release = 19)).to_pandas()
        if (sdss_table is None) | (sdss_table.empty):
            pass
        else:
            sdss_sigma = sdss_table['velDisp'].iloc[0] # SDSS velocity dispersion in km/s
            sdss_sigma_err = sdss_table['velDispErr'].iloc[0] # SDSS velocity dispersion error in km/s
        ...
        labels = {'cat': 'CaT', 'nai': 'NaI', 'mgi': 'MgI', 'all': 'Full', '5kto6k': r'$5000-6000\AA$'}

        for key, value in lam_lims.items():
            if (value[1] <= max(lam_gal_org)) & (value[0] >= min(lam_gal_org)):
                ...
                if name not in os.listdir(f'/home/ether/ether-ppxf/out/fitting_pipeline/{sample}/plots'):
                    os.mkdir(f'/home/ether/ether-ppxf/out/fitting_pipeline/{sample}/plots/{name}')
                ...
                plt.title(f"{name} — Wavelength region: {labels.get(key, '?')} — Predicted WISE $M_{{BH}}$: {mbh:.2f} [log $M_\\odot$]")
                ...
                plt.legend(loc = 'best')
                plt.grid(alpha = 0.3)
                plt.savefig(f'/home/ether/ether-ppxf/out/fitting_pipeline/{sample}/plots/{name}/sigma-fitting-{name}-{key}.png', transparent = False)
        ...
        if good_range != []:
            ...
            colors = {'cat': 'royalblue', 'nai': 'forestgreen', 'mgi': 'darkorange', 'all': 'gray', '5kto6k': 'purple'}
            markers = {'cat': 'o', 'nai': 's', 'mgi': '^', 'all': 'x', '5kto6k': 'D'}
            ...
            for key, pdata in plot_data.items():
                ...
                plt.axhline(
                    ...
                    linestyle='--',
                    linewidth=1,
                    label=fr"{labels.get(key, '?')} $\sigma_{{conv}}$ = {pdata['converg_sigma_final']:.2f}"
                )
                ...
                if (sdss_table is None) | (sdss_table.empty):
                    ...
                    plt.axhspan(
                        sdss_sigma - sdss_sigma_err,
                        sdss_sigma + sdss_sigma_err,
                        color = 'red',
                        alpha = 0.2,
                        label = fr"$\sigma_{{SDSS}} = {sdss_sigma:.2f} \pm {sdss_sigma_err:.2f}$"
                    )
            ...
            plt.tight_layout()
            plt.savefig(f'/home/ether/ether-ppxf/out/fitting_pipeline/{sample}/plots/{name}/sigma-guesses-pixels-{name}-combined.png', transparent=False)
            plt.close()
        
        # Convert numpy arrays to lists for CSV storage
        for key in plot_data.keys():
            for key2 in plot_data[key]:
                try:
                    plot_data[key][key2] = list(plot_data[key][key2])
                except TypeError:
                    pass

        # Save results to CSV and flush to disk
        to_ether = pd.DataFrame({'ONAME': name, 'RA': [header['RA']], 'DEC': [header['DEC']], f'PPXF{sample.upper()}': [plot_data]})
        to_ether.to_csv(results, index = False, header = False)
        results.flush()
